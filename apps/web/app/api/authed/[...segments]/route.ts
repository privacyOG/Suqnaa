...
const disputeEvidenceMaximumRequestBodyBytes = 10 * 1024 * 1024;
const maximumChallengeLength = 2048;
const supportedBinaryImageTypes = new Set(['image/jpeg', 'image/png', 'image/webp']);
...
function isDisputeEvidenceUploadRoute(route: NonNullable<ReturnType<typeof resolveProtectedRoute>>): boolean {
...
function isBinaryMediaUploadRoute(route: NonNullable<ReturnType<typeof resolveProtectedRoute>>): boolean {
  return /^\/v1\/listings\/[0-9a-fA-F-]+\/media\/upload$/.test(route.path) ||
    ...
    isDisputeEvidenceUploadRoute(route);
}

function binaryMediaMaximumRequestBodyBytes(route: NonNullable<ReturnType<typeof resolveProtectedRoute>>): number {
  if (isDisputeEvidenceUploadRoute(route)) return disputeEvidenceMaximumRequestBodyBytes;
...
async function prepareRequest(request: NextRequest, context: RouteContext): Promise<PreparedRequest | NextResponse> {
  ...
  if (challenge && challenge.length > maximumChallengeLength) return errorResponse('Human-check response is too long', 400);
  if (route.method === 'GET') return { route, challenge };

  if (isBinaryMediaUploadRoute(route)) {
    const contentType = normalizedContentType(request.headers.get('content-type'));
    const allowedTypes = isDisputeEvidenceUploadRoute(route) ? supportedDisputeEvidenceTypes : supportedBinaryImageTypes;
    if (!contentType || !allowedTypes.has(contentType)) return errorResponse('Unsupported binary content type', 415);
    const body = await request.arrayBuffer();
    if (body.byteLength === 0) return errorResponse('Binary body is required', 400);
    if (body.byteLength > binaryMediaMaximumRequestBodyBytes(route)) return errorResponse('Request body is too large', 413);
    return { route, challenge, body, contentType };
  }

  const rawBody = await request.text();
  if (new TextEncoder().encode(rawBody).byteLength > maximumJsonBodyBytesForRoute(route)) return errorResponse('Request body is too large', 413);
  let parsed: unknown;
  try { parsed = rawBody ? JSON.parse(rawBody) : {}; }
  catch { return errorResponse('Invalid JSON body', 400); }
  if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) return errorResponse('JSON body must be an object', 400);
  return { route, challenge, body: JSON.stringify(parsed), contentType: 'application/json' };
}

async function callApi(prepared: PreparedRequest, accessToken: string, userAgent: string | null): Promise<Response> {
  ...
  return fetch(`${apiBaseUrl}${prepared.route.path}${suffix}`, {
    ...
    headers: {
      accept: isProtectedBinaryDeliveryRoute(prepared.route)
        ? 'image/jpeg,image/png,image/webp,application/pdf,application/octet-stream'
        : 'application/json',
      authorization: `Bearer ${accessToken}`,
      'user-agent': userAgent ?? 'SuqnaaWeb/1.0',
      ...(prepared.route.method === 'POST' && prepared.contentType ? { 'content-type': prepared.contentType } : {}),
      ...(prepared.challenge ? { 'x-suqnaa-human-check': prepared.challenge } : {})
    },
...
async function upstreamResponse(
  ...
  const body = isJson ? await apiResponse.text() : await apiResponse.arrayBuffer();
  if (!isJson && body instanceof ArrayBuffer) headers.set('Content-Length', String(body.byteLength));
  const response = new NextResponse(isJson ? (body as string || null) : body as ArrayBuffer, { status: apiResponse.status, headers });
  if (apiResponse.status === 401) clearWebSessionCookies(response);
  else if (credentials) setWebSessionCookies(response, credentials);
  return response;
}
...
async function handleProtectedRequest(request: NextRequest, context: RouteContext): Promise<NextResponse> {
  ...
  if (!accessToken) {
    ...
    if (!rotation.ok) {
      ...
      if (rotation.status === 401) clearWebSessionCookies(response);
      return response;
    }
  ...
  try { apiResponse = await callApi(prepared, accessToken, userAgent); }
  catch {
    const response = errorResponse('Marketplace service unavailable', 503);
    if (rotatedCredentials) setWebSessionCookies(response, rotatedCredentials);
    return response;
  }
  ...
  if (apiResponse.status === 401 && !rotatedCredentials) {
    ...
    rotatedCredentials = rotation.credentials;
    try { apiResponse = await callApi(prepared, rotation.credentials.accessToken, userAgent); }
    catch {
      const response = errorResponse('Marketplace service unavailable', 503);
