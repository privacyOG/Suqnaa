...
const moderatorApprovalHook = readFileSync(new URL('./moderator-listing-approval-hook.ts', import.meta.url), 'utf8');
...
assert.match(moderatorApprovalHook, /body\?\.action !== 'approve'/);
assert.match(moderatorApprovalHook, /evaluateListingModerationPolicy/);
assert.match(moderatorApprovalHook, /policy\.decision !== 'block'/);
assert.match(moderatorApprovalHook, /listing_policy_blocked/);

assert.match(legacyQueueHook, /applyListingModerationAction/);
assert.match(legacyQueueHook, /applyAccountModerationAction/);
assert.match(legacyQueueHook, /evaluateListingModerationPolicy/);
assert.match(legacyQueueHook, /listing_policy_blocked/);
...
assert.match(server, /enforceModeratorListingApprovalPolicy/);
...
assert.match(server, /app\.addHook\('preHandler', enforceModeratorListingApprovalPolicy\)/);
