...
const { orientedDimensions, outputDimensions, inputSpecifier } = listingImageTransformInternals;
...
assert.equal(inputSpecifier('image/jpeg'), 'jpeg:-');
assert.equal(inputSpecifier('image/png'), 'png:-');
assert.equal(inputSpecifier('image/webp'), 'webp:-');
