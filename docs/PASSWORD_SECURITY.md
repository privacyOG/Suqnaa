...
Password recovery is intentionally enumeration-safe. `POST /v1/auth/password/forgot` accepts exactly one email address or phone number and returns the same accepted response after a valid request regardless of whether that contact exists, the account is closed, the account has reached its durable issuance limit, or downstream delivery fails.

Phone recovery inputs are normalized to canonical E.164 before lookup. Local-only numbers are rejected because the server does not guess a country. See `PHONE_IDENTITIES.md` for the canonicalization and migration rules.
...
The web application provides localized English/Arabic forgot-password, reset-password, and signed-in password/session management screens. Recovery can use email or an explicit international phone number. Password change and revoke-all clear the HttpOnly web-session cookies after the server-side operation succeeds.

The mobile application provides native email/phone password recovery, password reset, password change, session listing, individual revocation, and revoke-all controls. When a production human-verification challenge is required for a recovery request, mobile opens the exact localized secure web recovery path without putting account credentials or reset tokens in the handoff URL.
