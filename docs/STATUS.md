...
- Signed-in web and mobile catalogue users can save the current normalized filter set as a named search; listing-detail views record recent history and expose independent Save and Watch actions.
...
- Mobile email/phone authentication, password recovery/reset, password/session security, native profile/business/privacy editing, password-confirmed account closure/deletion, secure profile handoff for avatar/export, account contact verification, native seller-verification status, bounded seller-verification initiation/handoff, listing creation, optimistic-concurrency listing editing, native expiry/renewal/inventory management, native saved/watch/recent discovery, saved-search management and notification acknowledgement, listing-photo galleries, conversation, order activity, checkout preparation, cancellation, and fulfilment controls with CI coverage.
...
- Cross-channel email, SMS, and push delivery for discovery or other marketplace events remains deferred to the durable notification-provider implementation; P0-16 delivers saved-search matches only to the protected in-app notification store.
...
50. Signed-in buyers can independently save or watch an active listing. If a referenced listing later becomes unavailable, discovery lists retain only a removable reference and do not expose inactive listing details.
51. Opening an active listing while signed in records a bounded recently viewed relationship with first/last view timestamps and a per-listing view count.
52. Web and mobile catalogue users can save the current normalized filter set as a named search. Semantically identical filters are unique per account and do not persist pagination cursor or sort-only presentation state.
53. A saved search starts matching from its creation time rather than sending a backlog of existing listings. Re-enabling a paused search resumes from the current time, and changing its filter definition resets the cursor and removes alerts generated for the previous definition.
54. The discovery worker matches new or updated active listings using the same text/category/condition/availability/price/currency/location/fulfilment semantics as live catalogue search, excluding the search owner's own inventory and suspended/closed sellers.
55. Saved-search matches create durable in-app notifications with read/unread state and per-search/listing deduplication. Web and mobile users can acknowledge one alert or mark all saved-search alerts read.
56. The discovery worker is a separately supervised process (`worker:discovery`) using a PostgreSQL advisory lock, bounded saved-search batches, `FOR UPDATE SKIP LOCKED`, and an ordered `(updated_at, listing_id)` checkpoint so equal-timestamp listings are not skipped at a batch boundary.
