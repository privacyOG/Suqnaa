...
- Bounded in-memory rate limiting for authentication, account verification burst protection, listings, listing media, messages, conversations, offers, market actions, fulfilment transitions, signed payment events, and protected activity reads.
...
- Live bilingual web registration, login, contact verification, account, public marketplace, listing detail, Sell, My Listings, seller photo galleries, marketplace activity, order history/detail, conversation inbox, message-history, Message seller, and Make offer interfaces.
...
- Mobile authentication, account contact verification, listing, listing-photo galleries, conversation, order activity, checkout preparation, secure web handoff, cancellation, and fulfilment controls with CI coverage.
...
7. Access and refresh credentials move into protected session storage appropriate to the client.
8. Sellers create listing drafts through a challenge-bound form. In challenge-enabled deployments, images are added afterward so each upload receives its own media-specific verification.
9. Sellers preview draft and published galleries through owner-only URLs, add images up to the eight-photo limit, and delete individual images unless the listing is sold or removed.
10. Mobile sellers can preview those same owner-only galleries. Native upload and deletion run only when challenge verification is disabled; otherwise the app opens the exact localized secure photo manager without placing credentials, listing IDs, or challenge values in the URL.
11. Mobile binary uploads inherit access-token refresh and one-retry session behavior while preserving the original bytes and protected content type.
12. Buyers can start the listing conversation or submit one idempotent pending offer.
13. Sellers review incoming offers and atomically accept or reject them.
14. Accepting reserves the listing and rejects competing pending offers.
15. Buyers may cancel only pending offers or create one order from an accepted offer.
16. Order participants, amount, currency, payment method, and listing are derived from persisted records rather than client input.
17. Order creation atomically establishes one payment intent and one fulfilment record without collecting funds.
18. Buyers may irreversibly cancel eligible unpaid orders, which cancels the accepted offer, reactivates the listing, and synchronizes the payment context.
19. When an approved provider integration is configured, a signed and time-bounded `payment.held` event may atomically move only a matching pending order and eligible payment intent to paid/held.
20. Payment-event retries are accepted only when the provider event identifier and semantic payload match the durable replay ledger; conflicting replays are rejected.
21. Buyers and sellers can open participant-only order history, detail, and payment-context views with lifecycle progress.
22. Eligible sellers can mark readiness for pickup or submit bounded shipment evidence on web or mobile after verified held payment.
23. Eligible buyers can confirm receipt on web or mobile, with an explicit notice that confirmation does not release funds.
24. Mobile native fulfilment mutations run only when challenge verification is disabled; challenge-enabled environments open the exact secure web order for browser verification without placing mobile credentials in the URL.
25. My Listings loads only that seller's records and supports the API's allowed state transitions.
26. Messages lists only conversations where the account is a participant.
27. Conversation threads load protected history, acknowledge reads, and send idempotent challenge-bound messages.
28. Expired access sessions rotate automatically and retry the protected request once.

## Next implementation targets

- Password recovery, password change, and security-session revocation flows across API, web, and mobile.
