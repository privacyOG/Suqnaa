...
12. After verified provider integration produces a paid order with held funds, eligible sellers can mark readiness for pickup or submit bounded shipment evidence on web or mobile.
13. Eligible buyers can confirm receipt on web or mobile, with an explicit notice that confirmation does not release funds.
14. Mobile native fulfilment mutations run only when challenge verification is disabled; challenge-enabled environments open the exact secure web order for browser verification without placing mobile credentials in the URL.
15. The Sell page creates a protected listing draft without exposing bearer tokens.
16. My Listings loads only that seller's records and supports the API's allowed state transitions.
17. Messages lists only conversations where the account is a participant.
18. Conversation threads load protected history, acknowledge reads, and send idempotent challenge-bound messages.
19. Expired access sessions rotate automatically and retry the protected request once.
